...
public final class MigrationFilenameParser {
    ...
    public static ParsedMigrationName parse(Path path) {
        String fileName = path.getFileName().toString();
        Matcher versioned = VERSIONED.matcher(fileName);
        if (versioned.matches()) {
            String version = versioned.group(1);
            validateVersion(version);
            return new ParsedMigrationName(version, normalizeDescription(versioned.group(2)));
        }
